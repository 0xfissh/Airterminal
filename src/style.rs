...
impl From<Icon> for char {
    fn from(icon: Icon) -> Self {
        match icon {
            ...
            Icon::User => '\u{F2BD}',  // Font Awesome user icon
            Icon::Login => '\u{F29D}', // Font Awesome sign-in icon
...
pub fn dashed_line(theme: &Theme) -> Stroke<'_> {
    let palette = theme.extended_palette();
...
pub fn error_container(theme: &Theme) -> container::Style {
    let palette = theme.extended_palette();

    container::Style {
        text_color: Some(palette.danger.base.text),
        background: Some(palette.danger.weak.color.scale_alpha(0.1).into()),
        border: Border {
            width: 1.0,
...
pub fn login_container(theme: &Theme) -> container::Style {
    let palette = theme.extended_palette();
    ...
    container::Style {
        ...
        shadow: Shadow {
            ...
            blur_radius: 10.0,
            color: Color::BLACK.scale_alpha(if palette.is_dark { 0.5 } else { 0.1 }),
        },
        ..Default::default()
    }
}

pub fn primary_button(theme: &Theme, status: widget::button::Status) -> widget::button::Style {
    let palette = theme.extended_palette();

    widget::button::Style {
        text_color: Color::WHITE,
        background: Some(match status {
            widget::button::Status::Active => palette.primary.base.color,
...
pub fn disabled_button(theme: &Theme) -> widget::button::Style {
    let palette = theme.extended_palette();
