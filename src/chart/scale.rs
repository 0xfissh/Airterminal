    ...
    fn draw(&self, frame: &mut Frame) {
        match self {
            AxisLabel::X(rect, label) => {
                ...
                if rect.x + rect.width < 0.0 || rect.x > frame_bounds.width {
                    return;
                }
...
impl canvas::Program<Message> for AxisLabelsX<'_> {
                    ...
                    } = *interaction
                    {
                        ...
                        if difference_x.abs() > 1.0 {
                            *last_position = cursor_position;
                            ...
                            let message = Message::XScaling(difference_x * 0.2, 0.0, false, super::ZOOM_SENSITIVITY_TRACKPAD);
                    ...
                    mouse::ScrollDelta::Pixels { y, .. } => {
                        let message = Message::XScaling(
                            *y,
                            ...
                            true,
                            super::ZOOM_SENSITIVITY_TRACKPAD,
                        );
    ...
    fn draw(
        ...
        let labels = self.labels_cache.draw(renderer, bounds.size(), |frame| {
            ...
            let mut all_labels: Vec<AxisLabel> = Vec::with_capacity(label_count + 1); // +1 for crosshair
                    ...
                    } = *interaction
                    {
                        ...
                        if difference_y.abs() > 1.0 {
                            *last_position = cursor_position;
                            ...
                            let message = Message::YScaling(difference_y * 0.4, 0.0, false, super::ZOOM_SENSITIVITY_TRACKPAD);
