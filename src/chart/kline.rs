use super::{
    Action, Basis, Caches, Chart, ViewState, Interaction, Message,
...
use data::chart::{
    Autoscale, ViewConfig, KlineChartKind, PlotConstants, PlotData,
...
impl Chart for KlineChart {
    ...
    fn view_indicators(&self, enabled: &[Self::IndicatorType]) -> Vec<Element<'_, Message>> {
        let chart_state: &ViewState = self.state();
        ...
        for selected_indicator in enabled {
            ...
            let is_hyperliquid_oi = matches!(
                (self.chart.ticker_info.as_ref().map(|t| t.exchange()), selected_indicator),
                (Some(Exchange::HyperliquidPerps), KlineIndicator::OpenInterest)
            );

            if is_hyperliquid_oi {
                continue;
            }

            // Hide Open Interest for all exchanges when Tick-based charts are selected
            if matches!(self.chart.basis, Basis::Tick(_))
                && matches!(selected_indicator, KlineIndicator::OpenInterest)
            {
                continue;
            }
    ...
    fn autoscaled_coords(&self) -> Vector {
        ...
        match &self.kind {
            KlineChartKind::Footprint { .. } => {
                let x_translation = 0.5 * (chart.bounds.width / chart.scaling) - (chart.cell_width / chart.scaling);
                // Add spacing from y-axis
                let spacing_offset = -60.0 / chart.scaling;
                Vector::new(
                    x_translation + spacing_offset,
                    self.data_source.latest_y_midpoint(|kline| (kline.high + kline.low) / 2.0),
                )
            },
    ...
    fn is_empty(&self) -> bool {
        match &self.data_source {
            PlotData::TimeBased(timeseries) => timeseries.data_points.is_empty(),
            PlotData::TickBased(tick_aggr) => tick_aggr.data_points.is_empty(),
        }
    }
}
...
pub struct KlineChart {
    ...
    last_tick: Instant,
    // Buffer realtime updates while view is panned away from latest
    paused_klines: Vec<Kline>,
    paused_trades: Vec<(u64, Vec<Trade>)>,
}
...
impl KlineChart {
    pub fn new(
        ...
        match basis {
            Basis::Time(interval) => {
                            ...
                            *indicator,
                            match indicator {
                                    ...
                                    if let Some(info) = &ticker_info {
                                        ...
                                        if matches!(ex, Exchange::BybitLinear | Exchange::BybitInverse | Exchange::BybitSpot | Exchange::HyperliquidPerps | Exchange::OkxLinear | Exchange::OkxInverse | Exchange::OkxSpot) {
                                            ...
                                            let total_map = timeseries
                                                .volume_data()
                                                .into_iter()
                                                .map(|(t, (b, s))| (t, (-1.0f32, b + s)))
                                                .collect();
                                            IndicatorData::Volume(Caches::default(), total_map)
                                        } else {
                                ...
                                KlineIndicator::OpenInterest => {
                                    // Keep structure but will be hidden at view layer for Hyperliquid and Tick-based
                                    IndicatorData::OpenInterest(Caches::default(), BTreeMap::new())
                                }
                                KlineIndicator::CVD => {
                                    IndicatorData::CVD(Caches::default(), timeseries.cvd_data())
                                }
                            },
                        )
                    })
                ...
                // Set initial translation to provide space from the y-axis
                let initial_translation = Vector::new(-60.0, 0.0);
                ...
                KlineChart {
                    ...
                    paused_klines: Vec::new(),
                    paused_trades: Vec::new(),
                }
            }
            Basis::Tick(interval) => {
                    ...
                    .map(|indicator| {
                        (
                            *indicator,
                            match indicator {
                                KlineIndicator::Volume => {
                                    // For tick-based charts keep split buy/sell
                                    IndicatorData::Volume(
                                        Caches::default(),
                                ...
                                KlineIndicator::CVD => {
                                    IndicatorData::CVD(Caches::default(), tick_aggr.cvd_data())
                                }
                            },
                        )
                    })
    ...
    pub fn update_latest_kline(&mut self, kline: &Kline) {
        // Pause if view is panned away from latest
        let is_paused = self.chart.translation.x * self.chart.scaling > self.chart.bounds.width / 2.0;

        if is_paused {
            self.paused_klines.push(kline.clone());
            return;
        } else if (!self.paused_klines.is_empty()) || (!self.paused_trades.is_empty()) {
            // Flush buffered data when resuming
            self.flush_paused_updates();
        }
        ...
        match self.data_source {
            PlotData::TimeBased(ref mut timeseries) => {
                ...
                {
                    // For Bybit and Hyperliquid adapters, show TOTAL volume in time-based charts
                    if let Some(info) = &self.chart.ticker_info {
                        ...
                        if matches!(ex, Exchange::BybitLinear | Exchange::BybitInverse | Exchange::BybitSpot | Exchange::HyperliquidPerps) {
                            data.insert(kline.time, (-1.0, kline.volume.0 + kline.volume.1));
                        } else {
                ...
                if let Some(IndicatorData::CVD(_, data)) =
                    self.indicators.get_mut(&KlineIndicator::CVD)
                {
                    *data = timeseries.cvd_data();
                };
    ...
    fn missing_data_task(&mut self) -> Option<Action> {
        match &self.data_source {
            PlotData::TimeBased(timeseries) => {
                ...
                if !self.fetching_trades.0 && exchange::fetcher::is_trade_fetch_enabled() {
                        ...
                        .min()
                    {
                        ...
                        let first_kline_after_gap = timeseries
                            ...
                            .map_or(kline_latest, |(time, _)| time.saturating_sub(1));
                ...
                // priority 2, Open Interest data (skip for Hyperliquid and for Tick-based charts)
                let is_hyperliquid = self
                    .chart
                    .ticker_info
                    .as_ref()
                    .is_some_and(|t| matches!(t.exchange(), Exchange::HyperliquidPerps));

                let is_tick_based = matches!(self.chart.basis, Basis::Tick(_));

                if !is_hyperliquid && !is_tick_based {
                    ...
                    timeseries.check_integrity(kline_earliest, kline_latest, timeframe)
                {
                    ...
                    let earliest =
                        missing_keys.iter().min().unwrap_or(&visible_earliest).saturating_sub(timeframe);
    ...
    pub fn set_tick_basis(&mut self, tick_basis: u64) {
        ...
        if let Some(indicator) = self.indicators.get_mut(&KlineIndicator::CVD) {
            *indicator = IndicatorData::CVD(Caches::default(), new_tick_aggr.cvd_data());
        }
    ...
    pub fn insert_trades_buffer(&mut self, trades_buffer: &[Trade], depth_update: u64) {
        ...
        let is_paused = self.chart.translation.x * self.chart.scaling > self.chart.bounds.width / 2.0;
        if is_paused {
            self.paused_trades.push((depth_update, trades_buffer.to_vec()));
            return;
        } else if (!self.paused_klines.is_empty()) || (!self.paused_trades.is_empty()) {
            // Flush buffered data when resuming
            self.flush_paused_updates();
    ...
    fn flush_paused_updates(&mut self) {
        // Flush trades ordered by their timestamps
        if !self.paused_trades.is_empty() {
            self.paused_trades.sort_by_key(|(t, _)| *t);

            for (t, trades) in self.paused_trades.drain(..) {
                match self.data_source {
                    PlotData::TickBased(ref mut tick_aggr) => {
                        let old_dp_len = tick_aggr.data_points.len();
                    ...
                    PlotData::TimeBased(ref mut timeseries) => {
                        timeseries.insert_trades(&trades, Some(t));
                    }
                }
            }
        }

        // Flush klines ordered by their time
        if !self.paused_klines.is_empty() {
            self.paused_klines.sort_by_key(|k| k.time);
            for kline in self.paused_klines.drain(..) {
                // Insert kline with a short mutable borrow
                if let PlotData::TimeBased(ref mut timeseries) = self.data_source {
                    timeseries.insert_klines(&[kline.clone()]);
                }

                // Update Volume indicator mapping
                if let Some(IndicatorData::Volume(_, data)) =
                    self.indicators.get_mut(&KlineIndicator::Volume)
                {
                    if let Some(info) = &self.chart.ticker_info {
                ...
                // Recompute CVD from current timeseries
                ...
                // Update chart labels
                if kline.time > self.chart.latest_x {
                    self.chart.latest_x = kline.time;
                }
                self.chart.last_price = Some(PriceInfoLabel::new(kline.close, kline.open));
            }
        }
    }
                    ...
                    if let Some(info) = &self.chart.ticker_info {
                        ...
                        if matches!(ex, Exchange::BybitLinear | Exchange::BybitInverse | Exchange::BybitSpot | Exchange::HyperliquidPerps) {
                            data.extend(
                                klines_raw
                                    .iter()
                                    .map(|k| (k.time, (-1.0f32, k.volume.0 + k.volume.1))),
                            );
                        } else {
                            data.extend(
                                klines_raw
                                    .iter()
                                    .map(|k| (k.time, (k.volume.0, k.volume.1))),
                            );
                        }
                    } else {
                        data.extend(
        ...
        if let Some(autoscale) = chart.layout.autoscale {
            match autoscale {
                Autoscale::CenterLatest => {
                    ...
                    // Add spacing from y-axis
                    let spacing_offset = -60.0 / chart.scaling;
                    chart.translation.x = x_translation + spacing_offset;
        ...
        match self.indicators.entry(indicator) {
            ...
            Entry::Vacant(entry) => {
                let data = match indicator {
                        ...
                        PlotData::TimeBased(timeseries) => {
                            if let Some(info) = &self.chart.ticker_info {
                                ...
                                if matches!(ex, Exchange::BybitLinear | Exchange::BybitInverse | Exchange::BybitSpot | Exchange::HyperliquidPerps) {
                                    ...
                                    IndicatorData::Volume(Caches::default(), timeseries.volume_data())
                                }
                            } else {
                                IndicatorData::Volume(Caches::default(), timeseries.volume_data())
                            }
                    ...
                    KlineIndicator::CVD => match &self.data_source {
                        PlotData::TimeBased(timeseries) => {
                            IndicatorData::CVD(Caches::default(), timeseries.cvd_data())
                        }
                        PlotData::TickBased(tick_aggr) => {
        ...
        if chart.layout.crosshair {
            let crosshair = chart.cache.crosshair.draw(renderer, bounds_size, |frame| {
                if let Some(cursor_position) = cursor.position_in(bounds) {
                    // Kline: supply a measure builder that returns bar count (default behavior)
                    let measure = |from: u64, to: u64| -> Option<String> {
                        match self.data_source {
                            PlotData::TimeBased(_) => {
                                let interval_ms = match self.chart.basis { Basis::Time(tf) => tf, _ => return None };
                                let diff = to.saturating_sub(from);
                                Some(format!("{} bars", (diff / interval_ms).max(1)))
                            }
                            PlotData::TickBased(ref tick_aggr) => {
                                let tick_diff = to.saturating_sub(from) / tick_aggr.interval;
                                Some(format!("{} bars", tick_diff.max(1)))
                            }
                        }
                    };
...
fn draw_footprint_kline(
    ...
    // Ensure minimum body height for visibility (minimum 1 pixel)
    let body_height = (y_open - y_close).abs().max(1.0);
    let body_y = if kline.open == kline.close {
        // For doji candles, center the line on the price
        y_open - body_height / 2.0
    } else {
        y_open.min(y_close)
    };

    frame.fill_rectangle(
        Point::new(x_position - (candle_width / 8.0), body_y),
        Size::new(candle_width / 4.0, body_height),
        body_color,
    );
    ...
    // Draw wick with minimum visibility
    let wick_height = (y_high - y_low).abs();
    if wick_height > 0.0 {
        ...
        // For single-price candles, draw a small horizontal line
        let marker_line = Stroke::with_color(
            Stroke {
                width: 1.0,
                ..Default::default()
...
fn draw_candle_dp(
    ...
    frame.fill_rectangle(
        Point::new(x_position - (candle_width / 2.0), body_y),
        Size::new(candle_width, body_height),
        body_color,
    );

    // Draw wicks only if there's a difference between high/low and open/close
    let wick_color = if kline.close >= kline.open {
        palette.success.base.color
    } else {
        palette.danger.base.color
    ...
    // Ensure minimum wick visibility (minimum 1 pixel)
    let wick_height = (y_high - y_low).abs().max(1.0);
    let wick_width = candle_width / 4.0;

    frame.fill_rectangle(
        Point::new(x_position - (wick_width / 2.0), y_high),
        Size::new(wick_width, wick_height),
        wick_color,
    );
}
