...
pub fn view<'a>(
        ...
        match basis {
            Basis::Time(selected_timeframe) => {
                ...
                if is_kline_chart {
                    ...
                    // Use Hyperliquid-specific intervals for Hyperliquid exchange
                    let intervals = if exchange == Exchange::HyperliquidPerps {
                        // Only show 500ms and 1s for Hyperliquid
                        vec![Timeframe::MS500, Timeframe::MS1000]
                    } else {
                        // Use standard heatmap intervals for other exchanges
    ...
    if selected_ticksize.is_some() {
        ...
        // For Hyperliquid heatmap, enforce 1x multiplier by only showing 1x option
        let is_hyperliquid_heatmap = matches!(
            modifiers,
            StreamModifier::Heatmap(_, _)
        ) && matches!(ticker_info.map(|(e, _)| e), Some(Exchange::HyperliquidPerps));

        if is_hyperliquid_heatmap {
            let ticksize = TickMultiplier(1);
            ticksizes_column = ticksizes_column.push(create_button(
                ticksize.to_string(),
                None,
                true,
            ));
