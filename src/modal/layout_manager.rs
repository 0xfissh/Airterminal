...
impl LayoutManager {
    ...
    pub fn next_layout(&self) -> Option<Layout> {
        let current_index = self.layout_order.iter().position(|&id| id == self.active_layout.id)?;
        let next_index = (current_index + 1) % self.layout_order.len();
        let next_id = self.layout_order[next_index];
        
        self.layouts.get(&next_id).map(|(layout, _)| layout.clone())
    }

    pub fn previous_layout(&self) -> Option<Layout> {
        let current_index = self.layout_order.iter().position(|&id| id == self.active_layout.id)?;
        let prev_index = if current_index == 0 {
            self.layout_order.len() - 1
        } else {
            current_index - 1
        };
        let prev_id = self.layout_order[prev_index];
        
        self.layouts.get(&prev_id).map(|(layout, _)| layout.clone())
    }
