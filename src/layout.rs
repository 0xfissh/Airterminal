...
use exchange::{TickMultiplier, Timeframe, adapter::Exchange};
        ...
        data::Pane::HeatmapChart {
            ...
            if let Some(ticker_info) = settings.ticker_info {
                ...
                let tick_size = settings
                    .tick_multiply
                    .unwrap_or(if ticker_info.exchange() == Exchange::HyperliquidPerps {
                        TickMultiplier(1)
                    } else {
                        TickMultiplier(10)
                    })
                ...
                // Coerce invalid previous basis for Hyperliquid to 1000ms (1s)
                let basis = if ticker_info.exchange() == Exchange::HyperliquidPerps {
                    let allowed_ms_500: u64 = Timeframe::MS500.into();
                    let allowed_ms_1000: u64 = Timeframe::MS1000.into();
                    match settings.selected_basis {
                        Some(Basis::Time(ms)) if ms == allowed_ms_500 || ms == allowed_ms_1000 => {
                            Basis::Time(ms)
                        }
                        _ => Basis::Time(allowed_ms_1000),
                    }
                } else {
                    settings
                        .selected_basis
