use crate::{
    ...
    modal::{
        self, StreamModifier,
        pane::settings::{heatmap_cfg_view, kline_cfg_view, timesales_cfg_view},
        pane::stack,
    },
...
pub enum Message {
    ...
    PanelInteraction(pane_grid::Pane, super::panel::Message),
...
impl State {
    ...
    pub fn stream_pair(&self) -> Option<(Exchange, Ticker)> {
        self.streams
            ...
            .map(|stream| match stream {
                ...
                | StreamKind::Kline { ticker, .. } => (ticker.exchange, *ticker),
            })
            .next()
    }
    ...
    pub fn set_content(
        ...
        let new_content = match content_str {
            "heatmap" => {
                let is_hyperliquid = matches!(ticker_info.exchange(), Exchange::HyperliquidPerps);
                if is_hyperliquid {
                    // Coerce previous invalid selections (e.g., 100ms) to 1000ms (1s) for Hyperliquid
                    let allowed_ms_500: u64 = Timeframe::MS500.into();
                    let allowed_ms_1000: u64 = Timeframe::MS1000.into();
                    let needs_reset = match self.settings.selected_basis {
                        Some(Basis::Time(ms)) => ms != allowed_ms_500 && ms != allowed_ms_1000,
                        _ => true,
                    };
                    if needs_reset {
                        self.settings.selected_basis = Some(Basis::Time(allowed_ms_1000));
                    }
                }
                let tick_multiplier = if is_hyperliquid {
                    Some(TickMultiplier(1))
                } else {
                    Some(TickMultiplier(5))
                };
    ...
    pub fn duplicate(&self) -> Self {
        ...
        let new_content = match &self.content {
            Content::Heatmap(_, _) => {
                if let Some(ti) = settings.ticker_info {
                    let tick_size = settings
                        .tick_multiply
                        .map_or(ti.min_ticksize, |tm| tm.multiply_with_min_tick_size(ti));
                    Content::new_heatmap(&self.content, ti, &settings, tick_size)
                } else {
                    Content::Starter
                }
            }
            Content::Kline(_, _) => {
                if let Some(ti) = settings.ticker_info {
                    let tick_size = settings
                        .tick_multiply
                        .map_or(ti.min_ticksize, |tm| tm.multiply_with_min_tick_size(ti));
                    let content_str = match self.content.chart_kind() {
                        Some(data::chart::KlineChartKind::Candles) => "candlestick",
                        Some(data::chart::KlineChartKind::Footprint { .. }) => "footprint",
                        None => "candlestick",
                    };
                    Content::new_kline(content_str, &self.content, ti, &settings, tick_size)
                } else {
                    Content::Starter
                }
            }
            Content::TimeAndSales(panel) => {
                if let Some(ti) = settings.ticker_info {
                    let cfg = settings
                        .visual_config
                        .and_then(|vc| vc.time_and_sales())
                        .unwrap_or(panel.config);
                    Content::TimeAndSales(TimeAndSales::new(Some(cfg), Some(ti)))
                } else {
                    Content::Starter
                }
            }
        ...
        // Build a fresh state with a new UUID
        State::from_config(new_content, streams, settings)
    }
}
...
impl Content {
    ...
    fn new_kline(
        ...
        let enabled_indicators = {
            // Determine available indicators, hiding Open Interest for Hyperliquid
            let is_hyperliquid = matches!(ticker_info.exchange(), Exchange::HyperliquidPerps);
            let mut available: Vec<KlineIndicator> =
                KlineIndicator::for_market(ticker_info.market_type()).to_vec();
            if is_hyperliquid {
                available.retain(|i| !matches!(i, KlineIndicator::OpenInterest));
            }
            // Hide Open Interest for Tick-based charts on all exchanges
            ...
            prev_indis.map_or_else(
                ...
                |indis| indis.into_iter().filter(|i| available.contains(i)).collect(),
            )
        };
        ...
        match self {
            ...
            Content::Heatmap(chart, indicators) => {
                let base = chart::view(chart, indicators, timezone)
                    .map(move |message| Message::ChartInteraction(pane, message));
