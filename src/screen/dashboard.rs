...
impl Dashboard {
        ...
        match message {
            ...
            Message::Pane(window, message) => {
                match message {
                    ...
                    pane::Message::ClonePane(pane) => {
                        // Duplicate the current pane via State::duplicate and split horizontally
                        if let Some(current_state) = self.get_pane(main_window.id, window, pane) {
                            let cloned_state = current_state.duplicate();

                            if let Some((new_pane, _)) = self
                                .panes
                                .split(pane_grid::Axis::Vertical, pane, cloned_state)
                            {
                    ...
                    pane::Message::BasisSelected(new_basis, pane) => {
                        if let Some(state) = self.get_mut_pane(main_window.id, window, pane) {
                            ...
                            if let Some((_exchange, ticker)) = state.stream_pair() {
                    ...
                    pane::Message::DeleteNotification(pane, idx) => {
                        if let Some(pane_state) = self.get_mut_pane(main_window.id, window, pane) {
                            if idx < pane_state.notifications.len() {
                            ...
                            .and_then(|state| {
                                ...
                                state.streams.iter().find_map(|stream| {
                                    if let StreamKind::DepthAndTrades { ticker } = stream {
                                        Some((ticker.exchange, *ticker, pane_id, *stream))
                                    } else {
                                        None
                                    }
                                })
    ...
    fn set_pane_ticksize(
        ...
        if let Some(state) = self.get_mut_pane(main_window, window, pane) {
            // If Hyperliquid Heatmap, enforce 1x multiplier regardless of selection
            let enforce_hl_1x = matches!(state.content, pane::Content::Heatmap(_, _))
                && state
                    .settings
                    .ticker_info
                    .is_some_and(|ti| matches!(ti.exchange(), Exchange::HyperliquidPerps));

            let effective_multiplier = if enforce_hl_1x {
                TickMultiplier(1)
            } else {
                new_tick_multiply
            };

            state.settings.tick_multiply = Some(effective_multiplier);
                ...
                match state.content {
                    pane::Content::Kline(ref mut chart, _) => {
                        chart.change_tick_size(
                            effective_multiplier.multiply_with_min_tick_size(ticker_info),
                        );

                        chart.reset_request_handler();
                    }
                    pane::Content::Heatmap(ref mut chart, _) => {
                        let tick_size = effective_multiplier.multiply_with_min_tick_size(ticker_info);
                        
                        chart.change_tick_size(tick_size);
                    }
                    _ => {
                ...
                if pane_state.matches_stream(stream) {
                    match &mut pane_state.content {
                        ...
                        pane::Content::TimeAndSales(panel) => {
                            panel.update_trades(trades_buffer, timezone);
                        }
                        _ => {
...
fn oi_fetch_task(
    ...
    let fetch_task = match stream {
        StreamKind::Kline { timeframe, ticker } => Task::perform(
            adapter::fetch_open_interest(ticker.exchange, ticker, timeframe, range)
                .map_err(|err| format!("{err}")),
            move |result| match result {
                Ok(oi) => {
