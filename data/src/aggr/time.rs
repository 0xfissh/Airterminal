...
impl TimeSeries {
    ...
    pub fn cvd_data(&self) -> BTreeMap<u64, f32> {
        let mut cvd = 0.0;
        self.data_points
            .iter()
            .map(|(time, dp)| {
                let delta = dp.kline.volume.0 - dp.kline.volume.1; // buy_volume - sell_volume
                cvd += delta;
                (*time, cvd)
            })
            .collect()
    ...
    pub fn min_max_price_in_range(&self, earliest: u64, latest: u64) -> Option<(f32, f32)> {
        ...
        for (_, data_point) in self.data_points.range(earliest..=latest) {
