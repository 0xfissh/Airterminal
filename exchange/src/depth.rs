...
impl<'de> serde::Deserialize<'de> for Order {
        ...
        D: Deserializer<'de>,
    {
        ...
        let parse_f32 = |val: &Value| -> Option<f32> {
            match val {
        ...
        let price = match &value {
            Value::Array(arr) => arr.first().and_then(parse_f32),
            Value::Object(map) => map.get("0").and_then(parse_f32),
            _ => None,
        }
        .ok_or_else(|| SerdeError::custom("Order price not found or invalid"))?;

        let qty = match &value {
            Value::Array(arr) => arr.get(1).and_then(parse_f32),
            Value::Object(map) => map.get("1").and_then(parse_f32),
            _ => None,
        }
