                                    ...
                                    match data {
                                        StreamData::Trade(de_trade) => {
                                            let trade = Trade::new(
                                                ...
                                                de_trade.is_sell,
                                                de_trade.price,
                                                contract_size.map_or(de_trade.qty, |size| de_trade.qty * size),
                                            );
...
pub async fn fetch_ticksize(
    ...
    for item in symbols {
        ...
        if let Some(quote_asset) = item["quoteAsset"].as_str()
            && quote_asset != "USDT" && quote_asset != "USD" && quote_asset != "USDC" && quote_asset != "FDUSD" {
                continue;
            }
...
pub async fn fetch_historical_oi(
    ...
    let (base_url, pair_str, weight) = match market {
        ...
        _ => {
            ...
            log::error!("{}", err_msg);
            return Err(StreamError::InvalidRequest(err_msg));
        }
    };
                ...
                trades.extend(csv_reader.records().filter_map(|record| {
                    record.ok().and_then(|record| {
                        ...
                        Some(Trade {
                            ...
                            price,
                            qty,
                            ..Trade::new(time, is_sell, price, qty)
                        })
                    })
                }));
            }
